...
float shannon_entropy_update(shannon_entropy_state_t *state, uint16_t input) {
	...
	if (state->count < SHANNON_WINDOW_SIZE) {
		...
		uint16_t old_idx = state->window[state->pointer];
		uint16_t te_in = state->freq[idx];
		uint16_t te_out = state->freq[old_idx];

		state->window[state->pointer] = idx;

		if (state->freq[old_idx] > 0)
			state->freq[old_idx]--;
		state->freq[idx]++;

		state->S +=
			(((float)PiMap[state->freq[idx]] - (float)PiMap[te_in]) +
		...
		if (te_in == 0) {
			...
			if (state->freq[old_idx] == 0 && te_out == 1)
				state->k--;
		}
	}
	...
	return (float)((float)(state->k) / SHANNON_WINDOW_SIZE) *
		   (state->S / 1000000.0f);
}
