...
uint8_t symbolic_dynamic_update(symbolic_dynamic_state_t *state, WFDB_Time x, int32_t xl, int32_t xh) {
    if (!state) return 0; // Return a default symbol on error

    // --- Step 1: Calculate DeltaRR using historical data ---

    // Get the delayed samples from the circular buffers
    WFDB_Time x_delayed = state->x_buffer[state->x_index];
    int32_t xl_delayed = state->xl_buffer[state->xl_index];
    ...
    int32_t delta_rr = x_delayed - xl_delayed;

    // --- Step 2: Calculate thresholds and determine the symbol ---

    int32_t thres1 = xh / 16; // xh / 16
    int32_t thres2 = xh / 8; // xh / 8
    int32_t thres3 = thres1 + thres2;
    int32_t thres4 = xh / 4; // xh / 4
    int32_t thres5 = thres4 + thres1;

    uint8_t sy;
    if (delta_rr < -thres4)      sy = 0;
    else if (delta_rr < -thres3) sy = 1;
    else if (delta_rr < -thres2) sy = 2;
    else if (delta_rr < -thres1) sy = 3;
    else if (delta_rr < thres1)  sy = 4;
    else if (delta_rr < thres2)  sy = 5;
    else if (delta_rr < thres3)  sy = 6;
    else if (delta_rr < thres4)  sy = 7;
    else if (delta_rr < thres5)  sy = 8;
    else                         sy = 9;
